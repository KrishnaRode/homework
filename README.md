...
> **Repo layout:** the PrepWell app lives in [`prepwell/`](prepwell). The root-level
> `project.md`, `PROMPT.md`, and `design.md` are the build spec/scaffolding it was
> generated from.
...
- **Question engine** (`prepwell/backend/app/question_engine`) — builds a prompt from the
  syllabus topic + the student's mental model, calls Ollama (strict JSON), validates,
  de-duplicates by `question_hash`, and rotates through 12 question types (never MCQ-only).
- **Evaluation engine** (`prepwell/backend/app/evaluation`) — deterministic checks for
  objective answers, an LLM rubric for subjective ones; scores 0–100 and explains how to think.
- **Mental model** (`prepwell/backend/app/mental_model`) — accuracy / speed / confidence /
  reasoning indices + per-topic mastery; decides the next difficulty and what to practice next.
- **Guardrails** (`prepwell/backend/app/guardrails`) — child-safety filtering,
  grade-appropriate language, JSON repair, and quality regeneration.
...
Backend env (`prepwell/backend/.env`, see `prepwell/backend/.env.example`):
...
Frontend env (`prepwell/frontend/.env.local`): `NEXT_PUBLIC_API_BASE=http://localhost:8000`.
...
```
homework/
├── prepwell/                       # the PrepWell application
...
├── project.md / PROMPT.md / design.md   # build spec + scaffolding
├── LICENSE                         # MIT
└── README.md
```
...
- **Syllabus:** Classes **1–10** Maths, Science, English, curated from the NCERT
  framework in `prepwell/backend/app/syllabus/curriculum_data.py`
...
- **Students:** `stu_001` (Aarav, CBSE) and `stu_002` (Diya, CBSE) ship with mental models
  in `prepwell/data/students/`. Admins pick the board when creating a student.
- The SQLite DB (`prepwell/data/prepwell.db`) is seeded automatically on first backend start.
...
- **More classes/subjects:** drop a normalized JSON into `prepwell/data/syllabus/` — no code change.
- **Scraping:** `prepwell/backend/app/syllabus/scraper.py` is a ready extension point; PrepWell never
  depends on live scraping to run.
- **New question types:** add to `config.QUESTION_TYPES` + a hint in
  `question_engine/prompts.py`.
- **New brain games:** add a generator in `prepwell/frontend/lib/brainGames.ts` and a component in
  `prepwell/frontend/components/games/`.
